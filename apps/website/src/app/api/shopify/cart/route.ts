...
export const GET = apiHandler(
  {
    errorMsg: "Failed to retrieve cart details",
    successMsg: "Cart details retrieved successfully",
  },
  async ({ req }) => {
    await connectDB();
      ...
      if (shopifyCart.cart) {
        ...
        return {
          ...
          error: {
            type: "bad-request",
            message:
              "Cart not found! Please create a cart first with cartId " +
              cartId,
          },
        };
      }
...
export const POST = apiHandler(
  {
    errorMsg: "Failed to create cart",
    successMsg: "Cart created successfully",
  },
  async ({ req }) => {
    const session = await getSession();
